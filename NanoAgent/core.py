...
class NanoAgent:
    def __init__(self,api_key:str,base_url:str,model:str,max_tokens:int,actions=[],debug=False,retry=20):                
        ...
        self.model=model
        self.sysprmt=f'you are a helpful assistant, your task is to destructively analyze the user request with reasoning from abstractive concepts to concrete examples, then output step by step execution for the user query, MUST end every anwser with an action from {self.actions} until the answer is the final result.'
